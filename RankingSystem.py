...
def main():
    ...
    try:
        ranking_system = RankingSystem(folder_path)
        print(f"Loaded {len(ranking_system.documents)} documents.")
        ...
        while True:
            ...
            if query:
                if method == '1':
                    ranked_docs = ranking_system.rank_by_keyword_matching(query)
                elif method == '2':
                    ranked_docs = ranking_system.rank_by_tfidf(query)
                elif method == '3':
